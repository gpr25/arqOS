...
**2026-07-22 — Marketing / Registro semanal de mídia paga multi-empresa (cross-business)**
**Decisão/Aprendizado:** desenhada automação de leitura semanal de Google Ads + Meta Ads e registro na planilha mestre **"GADENS — Hub de Performance Semanal"** (Sheet ID `1bu_FXc779xE0Uu100Fclzs9uudqIe2I55Z4xojFobz4`), compartilhada pelas 5 empresas do Gabriel — não só Triunfo. Por ser cross-business, tudo ficou em `~/.claude` (não dentro desta pasta) — `~/.claude/commands/registrar-midia-paga.md` + `~/.claude/loops/midia-paga/`. Passou por três arquiteturas até chegar na atual: (1) webhook Apps Script — descartado; (2) automação via navegador — testada e descartada (sem sessão autenticada); (3) **atual**: Gabriel autorizou o `gws` (Google Workspace CLI), que escreve direto na planilha via Sheets API (`values.batchUpdate`, upsert por Semana+Plataforma), e optou por integração direta com Google Ads API + Meta Marketing API (em vez de Supermetrics ou export manual) para a leitura. Scripts em `~/.claude/loops/midia-paga/scripts/` (`pull-google-ads.ps1`, `pull-meta-ads.ps1`, `write-week-to-sheet.ps1`, `run-weekly.ps1`).
**Contexto:** mapeada a estrutura real de todas as abas da planilha (cada empresa/segmento tem colunas ligeiramente diferentes — ex.: Shopping Cidade Google não tem "Alcance" mas tem "Parcela de Impressões de Pesquisa"; colunas de fórmula como CTR/CPC/CPM/CPL/Taxa Conv. nunca são sobrescritas). Credenciais das APIs de anúncio devem ficar em `%USERPROFILE%\.config\gadens-ads\` — **nunca dentro de um repositório git** (esta pasta e Grupo Gadens são repos).
**Por quê:** Gabriel quer a rotina 100% automatizada, sem export manual semanal, cobrindo as 5 empresas que gerencia.
**Impacto:** Pendência bloqueante: Gabriel precisa preencher `google-ads.json` e `meta-ads.json` com credenciais reais (developer token, refresh token, account IDs — passo a passo em `~/.claude/loops/midia-paga/setup-apis.md`). Primeira execução real é calibragem, não produção. Depois de validado: backfill combinado (Shopping Cidade semanas de 29/06 e 06/07; Triunfo a partir de 06/07) e agendamento via skill `schedule`. Pedido adicional do Gabriel, ainda não iniciado: processo de relatório + apresentação mensal por empresa a partir desses dados.
