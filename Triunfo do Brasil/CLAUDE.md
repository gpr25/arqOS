...
Ver lista completa em `../Geral/README.md` (documentação do ArqOS) — destaque para `/gerar-copy`, `/anuncio-google`, `/gerar-campanha`, `/relatorio-ads` e `/analisar-dados`, mais relevantes para a operação de mídia paga da Triunfo. `/analisar-criativo` também está disponível (comando global, não específico da Triunfo — ver `~/.claude/commands/`).
